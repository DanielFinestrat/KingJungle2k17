...
void Temporizador::restartGame() {
    ...
    if (Partida::getInstance()->rondas.at(Partida::getInstance()->rondaActual).modoJuego == 0) {
        ...
        int posGanador = 0;
        for (int i = 0; i < partida->worldPlayer.size(); i++) {
            if(partida->worldPlayer.at(i)->getMonedero() > maxMonedero){ 
                maxMonedero = partida->worldPlayer.at(i)->getMonedero();
                posGanador = i;
            }
            else if(partida->worldPlayer.at(i)->getMonedero() == maxMonedero){
                posGanador = -1;
            }
        }
        for(int i = 0; i<partida->worldPlayer.size(); i++){
            if(i != posGanador) partida->players2Delete.push_back(partida->worldPlayer.at(i));
        }
    }

    partida->finalLevelTextPrepared = true;
    partida->notFirstReset = true;
    partida->timeBetweenReset = 0;
    partida->changeLevelClock.restartClock();
    stop(true);
