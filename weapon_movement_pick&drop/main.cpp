                    ...
                    switch (event.key.code) {
                        case Keyboard::Space:
                            if (weapon->inPossession && weapon->ammo > 0) {
                                weapon->dt = weapon->deltaClock.restart(); //Reiniciamos el clock
                                weapon->difTime += weapon->dt.asMilliseconds(); //Sumamos al contador de tiempo

                                //Comprobamos que ha pasado el tiempo suficiente para poder volver a disparar
                                if (weapon->difTime >= (1 / weapon->shootCadence) * 1000) {
                                    weapon->difTime = 0.0; //Dejamos la diferencia a 0 para resetear el tiempo entre disDisparos
                                    weapon->shoot(world); //Disparo
                                    weapon->ammo--; //Disminuimos la municion del arma
                                    weapon->lastShot = time(NULL); //Tiempo del ultimo tiro
